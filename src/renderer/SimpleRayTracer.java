...
//        if you get close to 0 –it’s time to get out( return 0)
    ...
    /**
     * calculate color at given point using phong (kA*iA(iE + sum((kD*abs(lI*n)+kS*(max(0, -v*r))^Sh)*iLi)))
            ...
            if ((nl* nv> 0) && unshaded(gp, l, n, lightSource)){
                ...
                if (!ktr.product(k).lowerThan(MIN_CALC_COLOR_K)) {
                    ...
                    color = color.add(iL.scale(calcDiffusive(material.kD, nl, intensity)
                            .add(calcSpecular(material.kS, material.shininess, n, l, nl, v, intensity))));
                }
            }
        }
        return color;
